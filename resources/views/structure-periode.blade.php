...
@section('title', 'Struktur Periode')

@section('content')
<div class="max-w-6xl mx-auto px-4 py-10">
    <div class="mb-10 text-center">
        <h1 class="text-3xl md:text-4xl font-bold text-red-700 mb-2">Struktur Organisasi DPM</h1>
        <p class="text-gray-600 text-lg">Periode: <span class="font-semibold text-blue-700">{{ $periode }}</span></p>
    </div>

    @if($anggota->isEmpty())
        <p class="text-center text-gray-500">Belum ada data struktur untuk periode ini.</p>
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            @foreach ($anggota as $item)
                <div class="bg-white rounded-lg shadow hover:shadow-lg transition p-5 text-center">
                    ...
                    <p class="text-sm text-gray-700 font-medium">{{ $item->jabatan }} ({{ $item->bagian }})</p>
